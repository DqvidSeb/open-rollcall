...
export function DepartmentsTable({ onRowClick }: DepartmentsTableProps) {
  ...
  const { data, isLoading, error } = useDepartments({ page, limit });
  ...
  const someChecked = items.some(d => selected.has(d.id)) && !allChecked;

  const toggleAll = () => {
    if (allChecked) {
      setSelected(prev => { const n = new Set(prev); items.forEach(d => n.delete(d.id)); return n; });
    } else {

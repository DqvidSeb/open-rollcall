...
class AttendanceRepository(BaseRepository[AttendanceLog]):
    ...
    async def get_today_event(self, employee_id: uuid.UUID, event_type: EventType) -> AttendanceLog | None:
    ...
    async def list_by_employee(self, employee_id: uuid.UUID, *, date_from: datetime | None = None,
                                date_to: datetime | None = None, offset: int = 0, limit: int = 50) -> list[AttendanceLog]:
        filters = [AttendanceLog.employee_id == employee_id]
        if date_from:
    ...
    async def count_by_employee(self, employee_id: uuid.UUID, date_from: datetime | None = None,
